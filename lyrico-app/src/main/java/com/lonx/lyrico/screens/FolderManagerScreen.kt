...
fun FolderManagerScreen(
    ...
    // WearOS 内置文件选择器状态（替代系统 OpenDocumentTree）
                        ...
                        SmallTopAppBar(
                            ...
                            actions = {
                                if (currentFolderId == ROOT_FOLDER_ID) {
                                    IconButton(
                                        onClick = {
                                            // 直接使用 WearOS 内置文件选择器（Shizuku 授权）
                                            // 不再依赖系统 DocumentsUI（手表上不可用）
                                            showFilePicker = true
                                        }
                                    ) {
                                        Icon(
    ...
    // 手表无 DocumentsUI，用 Shizuku + 自建文件浏览器选择音乐文件夹
    if (showFilePicker) {
        Dialog(
            ...
            FilePickerScreen(
                ...
                title = stringResource(R.string.wear_select_folder_title),
                onFileSelected = { },
